const rwtMdl = require("../../../models/settings/role-wise-tabs")

exports.add = async (req, res) => {
 try {
  const reqParams = req["body"] || {}
  const result = await rwtMdl.add(reqParams)
  if (result["status"]) {
...
exports.update = async (req, res) => {
 try {
  const reqParams = req["body"] || {}
  const result = await rwtMdl.update(reqParams)
  if (result["status"]) {
   return res.status(SUCCESS_CODE).json({ "status": result["status"], "msg": result["msg"], "data": result["data"] })
  } else {
   return res.status(DUPLICATE_ENTRY_CODE).json({ "status": result["status"], "msg": result["msg"] })
...
exports.details = async (req, res) => {
 try {
  const reqParams = req["body"] || {}
  const result = await rwtMdl.details(reqParams)
...
exports.del = async (req, res) => {
 try {
  const reqParams = req["body"] || {}
  const result = await rwtMdl.del(reqParams)
  if (result["status"]) {
   return res.status(SUCCESS_CODE).json({ "status": result["status"], "msg": result["msg"], "data": result["data"] })
  } else {
   return res.status(DUPLICATE_ENTRY_CODE).json({ "status": result["status"], "msg": result["msg"] })
